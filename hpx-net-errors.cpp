...
hpx::lcos::future<float> future_productsum(std::vector<hpx::lcos::future<float>>,std::vector<float>);
...
class network
{
	...
	void init(int in, int hidden_rows, int hidden_cols, int out, int bias = 0,int ctrl = 0)
	{
		...
		for(int i = 0; i < hidden_rows; i++) //"Rows" in Hidden Layers
		{
			...
			for(int j = 0; j < hidden_cols; j++) //"Columns" in Hidden Layers
			{
				...
				for(int k = 0; k < (int)row[i].size(); k++)
				{
					...
					else    hidden_layer[j].weights.push_back(debug);
		...
		for(int i = 0; i < out; i++)
		{
			...
			for(int k = 0; k < hidden_cols + 1; k++)
			{
				...
				else    output_layer[i].weights.push_back(debug);
	...
	{
		this->init(in,hidden_rows,hidden_cols,out,bias,1);
	}
	void profile()
	{
		std::cout << "\n";
...
//XOR
	...
	for(int i = 0; i < 5000; i++)
	{
		...
		for(int it = 0; it < (int)n.rows[n.rows.size()-1].contents.size(); it++)
		{
			...
			r = round(r);
			std::cout << r << " ";
			if(r != targets[s][counter]) valid = 0;
			else counter++;
		}
